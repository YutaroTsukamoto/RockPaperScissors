...
def run_yolo_detection(camera_id: int):
    """
    Webカメラからリアルタイムで映像を取得し、YOLOv8モデルでじゃんけんの手を検出する。
    検出結果は都度 process_detection_results 関数に渡される。
    """
    # ロガーの設定
    ...
    if not cap.isOpened():
        logging.error("エラー: カメラを開けませんでした。")
        return

    print("YOLOv8による検出を開始します。'q'キーで終了します。")
    logging.info("検出を開始しました。")

    while True:
        ret, frame = cap.read()
        if not ret:
            logging.error("エラー: フレームを読み込めませんでした。")
            break
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                # 検出結果をログファイルに記録
                logging.info(f"検出: {detection_data}")

                # 映像にバウンディングボックスとラベルを描画
                label = f'{class_name} {confidence:.2f}'
                cv2.rectangle(frame, (x1, y1), (x2, y2), (255, 0, 255), 3)
                cv2.putText(frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 255), 2)

        cv2.imshow('YOLOv8 Janken Detection', frame)
    ...
    cap.release()
    cv2.destroyAllWindows()
    logging.info("検出を終了しました。")
    print("検出を終了しました。")


def main():
    parser = argparse.ArgumentParser(description='じゃんけんの手を検出します。')
    parser.add_argument('--mode', type=str, default='mediapipe', choices=['mediapipe', 'yolo'], help='検出モードを選択 (mediapipe or yolo)')
    parser.add_argument('--video', type=str, help='動画ファイルのパス (mediapipeモードのみ)')
    parser.add_argument('--camera', type=int, default=0, help='カメラデバイスのID')
    ...
    if args.mode == 'mediapipe':
        ...
        if args.video:
            ...
            extractor.extract_landmarks_from_camera(args.output)
    elif args.mode == 'yolo':
        run_yolo_detection(args.camera)
    else:
        print("無効なモードが選択されました。'mediapipe'または'yolo'を選択してください。")
